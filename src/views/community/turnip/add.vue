<template>
  <v-container class="bgWht" elevation-4>
    ...
    <list-header title="发布我的大头菜信息" :tips="tips" :has-tip="true"> </list-header>
    <v-form ref="addForm" v-model="valid" lazy-validation class="turnip-form pa-16">
      <v-row>
        <v-col cols="4">
          <v-text-field v-model="newTurnip.price" :rules="formRules.require" label="菜价" placeholder="请输入" required></v-text-field>
        </v-col>
        <v-col cols="4">
          <v-radio-group v-model="newTurnip.exchangeType" label="交易类型" :rules="formRules.require" row>
        ...
        <v-col v-if="isLineBL && isAutoBL" cols="4">
          <v-text-field v-model="newTurnip.maxPeople" label="最大登岛人数限制" type="number"></v-text-field>
        </v-col>
        <v-col v-if="isLineBL && isAutoBL" cols="4">
          <v-text-field v-model="newTurnip.maxTime" label="登岛超时时间(分钟)" type="number"></v-text-field>
        </v-col>
        ...
        <v-col v-if="!isPsw" cols="4">
          <v-text-field v-model="newTurnip.contactDetail" :label="newTurnip.contact" :placeholder="'请输人' + contact" required></v-text-field>
        </v-col>
        <v-col v-if="isPsw" cols="4">
          <v-text-field v-model="newTurnip.psw" label="开岛密码" placeholder="请输入" required></v-text-field>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12">
