<template>
  <v-container class="bgWht" elevation-4>
    ...
    <list-header title="交易区" :tips="tips" :has-tip="true">
      <template v-slot:btn>
        <v-btn class="primary mt-1" href="/community/trade/add">发布我的交易信息</v-btn>
      </template>
    </list-header>
    <v-data-iterator
      :items="list"
      ...
      <template v-slot:default="props">
        <v-row>
          <v-col v-for="item in props.items" :key="item.name" cols="12" sm="4">
            <v-card :href="'/detail/trade/' + item._id">
              ...
              <v-card-text class="text--primary">
                <div class="d-flex justify-lg-space-between">
                  ...
                  <div class="subtitle-1 grey--text text--darken-1 line-40">登岛人数：{{ item.maxPeople }}</div>
                </div>
                ...
                <div class="d-flex justify-space-between">
                  <v-btn class="cyan--text text--lighten-2" text
                    ><v-icon class="mr-1" small>mdi-contactless-payment-circle</v-icon>{{ item.isLineup === true ? '立即上岛' : '立即联系' }}</v-btn
                  >
                  <v-btn class="cyan--text text--lighten-2" text
...
export default {
  ...
  methods: {
    getList() {
      ...
      getTradeList(queryInfo).then(response => {
