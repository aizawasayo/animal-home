<template>
  <v-container class="bgWht picList" elevation-4>
        ...
        <v-row>
          ...
          <v-col cols="12" class="py-10">
            <div class="mx-auto px-10">
              <v-chip v-if="info.exchangeType" :color="info.exchangeType.indexOf('特殊NPC') > -1 ? 'primary' : 'secondary'" text-color="white">
                {{ info.exchangeType }}
              </v-chip>
              {{ info.detail }}
            </div>
    ...
    <comment type="trade" default-text="有想法可以留言哦～"></comment>
  </v-container>
</template>
