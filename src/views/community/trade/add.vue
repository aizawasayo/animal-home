<template>
  <v-container class="bgWht" elevation-4>
    ...
    <list-header title="发布我的交易信息" :tips="tips" :has-tip="true"> </list-header>
    <v-form ref="addForm" v-model="valid" lazy-validation class="turnip-form pa-16">
      <v-row>
        <v-col cols="4">
          <!-- <cascader-multi v-model="newTrade.exchangeType" @on-change="onchange" :data="" multiple></cascader-multi> -->
          <Cascader :data="exchangeOption" v-model="newTrade.exchangeTypeArray" placeholder="请选择交易类型"></Cascader>
        </v-col>
        <v-col cols="4">
          <TimePicker
            type="time"
      ...
      <v-row>
        <v-col cols="4">
          <v-switch v-model="newTrade.isLineup" :label="`排队模式：${newTrade.isLineup ? '是' : '否'}`" color="primary" class="mt-0 pt-0"></v-switch>
        </v-col>
      ...
      <v-row>
        <v-col v-if="isLineBL && isAutoBL" cols="4">
          <v-text-field v-model="newTrade.maxPeople" label="最大登岛人数限制" type="number"></v-text-field>
        </v-col>
        <v-col v-if="isLineBL && isAutoBL" cols="4">
          <v-text-field v-model="newTrade.maxTime" label="登岛超时时间(分钟)" type="number"></v-text-field>
        </v-col>
        <v-col v-if="!isLineBL" cols="4">
        ...
        <v-col v-if="!isLineBL" cols="4">
          <v-text-field v-model="newTrade.contactDetail" :label="newTrade.contact" :placeholder="'请输人' + contact" required></v-text-field>
        </v-col>
        <v-col v-if="isLineBL" cols="4">
          <v-text-field v-model="newTrade.psw" label="开岛密码" placeholder="请输入" required></v-text-field>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12">
  ...
  methods: {
    ...
    submit() {
      ...
      if (!this.newTrade.validTime) {
        return this.$root.$dialogLoader.showSnackbar('请选择截止时间', { color: 'error' })
      }
