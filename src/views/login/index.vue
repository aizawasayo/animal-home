  ...
  <v-container class="bgWht" elevation-4>
    ...
    <v-form ref="loginForm" v-model="valid" lazy-validation class="login-form">
      ...
      <div class="d-flex justify-lg-space-between mt-4">
        <v-btn :disabled="!valid" color="primary" class="mr-4" large @click="login">
          登录
        </v-btn>
        <v-btn color="secondary" class="mr-4" large @click="reset">
          重置
        </v-btn>
        <v-btn color="success" large @click="register">
          注册
        </v-btn>
      </div>
    </v-form>
...
export default {
  ...
  data() {
    return {
      ...
      loginRules: {
        username: [v => !!v || '用户名必填！'],
        psw: [v => !!v || '密码必填！', v => v.length >= 6 || '密码不能少于6位']
      },
      pswShow: false,
      breadNav: [
        {
    ...
    login() {
      if (this.$refs.loginForm.validate()) {
        ...
        this.$store
          ...
          .then(() => {
            this.$root.$dialogLoader.showSnackbar('登录成功！', { color: 'success' })
            if (this.redirect) {
              // 需登录页面传前路由带过来的路径
          ...
          .catch(() => {
            this.$root.$dialogLoader.showSnackbar('登录失败，请输入正确的用户名和密码！', { color: 'error' })
          })
      } else {
        this.valid = false
        this.$root.$dialogLoader.showSnackbar('请输入正确的用户名和密码！', { color: 'error' })
      }
    },
    reset() {
