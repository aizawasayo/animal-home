<template>
  <v-container class="bgWht" elevation-4>
    ...
    <list-header title="修改个人信息" :tips="tips" :has-tip="false"> </list-header>
    <v-form ref="addForm" v-model="valid" lazy-validation class="pa-16">
      <v-row>
        <v-col cols="4">
          <v-text-field v-model="userInfo.username" :rules="formRules.require" label="用户名" placeholder="请输入" disabled required></v-text-field>
        </v-col>
        <v-col cols="4">
          <v-text-field v-model="userInfo.nickname" :rules="formRules.require" label="昵称" placeholder="请输入" required></v-text-field>
        </v-col>
        <v-col cols="4">
          <v-text-field v-model="userInfo.gameId" :rules="formRules.require" label="动森ID" placeholder="请输入" required></v-text-field>
        </v-col>
        <v-col cols="4">
          <v-select v-model="userInfo.position" :items="positionList" label="岛屿位置" placeholder="请选择" :rules="formRules.require"> </v-select>
        </v-col>
        <v-col cols="4">
          <DatePicker type="date" v-model="userInfo.islandDate" placeholder="请选择登岛日期" style="width: 100%" @on-change="test"></DatePicker>
          <!-- <v-datetime-picker
            label="请选择登岛日期"
        ...
        <v-col cols="4">
          <v-text-field v-model="userInfo.islandName" :rules="formRules.require" label="岛屿名称" placeholder="请输入" required></v-text-field>
        </v-col>
        <v-col cols="4">
          <v-file-input
            accept="image/*"
