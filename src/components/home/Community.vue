<template>
  <v-card>
    <v-tabs v-model="communityTab" background-color="transparent" class="elevation-0 community">
      ...
      <v-tab v-for="(item, i) in communityTabs" :key="item + i" :href="`#${item.type}`" @click="tabClick($event, item.type)">
        {{ item.text }}
      </v-tab>
      <v-btn large text class="btn-more" color="white" @click="goDetail">更多<v-icon small right>mdi-more</v-icon></v-btn>
