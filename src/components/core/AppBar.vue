        ...
        <div v-for="(item, i) in menu" :key="'menu' + i" v-bind="item">
          <v-menu v-if="item.children" offset-y:open-on-hover="true">
            <template v-slot:activator="{ on, attrs }">
              <v-btn dark text large class="text-button" v-bind="attrs" v-on="on">
                {{ item.text }}
              </v-btn>
            </template>
            <v-list>
              <v-list-item v-for="(subNav, index) in item.children" :key="'subNav' + index" :href="subNav.href">
                <v-list-item-title>{{ subNav.text }}</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
          <v-btn v-else large text class="text-button" :href="item.href">
            {{ item.text }}
          </v-btn>
        </div>
        ...
        <template v-slot:activator="{ on, attrs }">
          <div v-bind="attrs" v-on="on">
            <v-avatar class="mr-2" style="flex: none;" width="40" height="40">
              <img :src="apiUrl + avatar" :alt="username" />
            </v-avatar>
            <span class="mr-2" style="flex:1; line-height:40px;">{{ username }}</span>
          </div>
        </template>
        <v-list>
          <v-list-item href="/user">
            <v-list-item-title> <v-icon class="mr-1">mdi-account-box-outline</v-icon>个人中心 </v-list-item-title>
          </v-list-item>
          ...
          <v-list-item>
            <v-list-item-title @click="logout"> <v-icon class="mr-1">mdi-logout-variant</v-icon>退出登录 </v-list-item-title>
          </v-list-item>
        </v-list>
      </v-menu>
    </div>
    <v-btn v-else text class="mr-2" href="/login">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['menu', 'apiUrl', 'token', 'username', 'avatar']) //这种最便利
  },
  methods: {
    async logout() {
      if (await this.$root.$confirm('提示', '确定退出登录吗', { color: 'warning' })) {
        this.$root.$dialogLoader.start(
          '登出中...',
          {},
          async () => {
            await this.$store.dispatch('user/logout')
            this.$root.$dialogLoader.showSnackbar('登出成功!', { color: 'success' })
            this.$router.go(0)
          },
          true
        )
